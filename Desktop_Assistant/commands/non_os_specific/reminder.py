...
def _parse_absolute_time(text: str) -> datetime | None:
    ...
    patterns = [
        (r"(\d{1,2}):(\d{2})\s*(am|pm)", lambda m: datetime.strptime(f"{m.group(1)}:{m.group(2)} {m.group(3).upper()}", "%I:%M %p")),
        (r"(\d{1,2})\s*(am|pm)",          lambda m: datetime.strptime(f"{m.group(1)} {m.group(2).upper()}", "%I %p")),
        (r"(\d{2}):(\d{2})",              lambda m: datetime.strptime(f"{m.group(1)}:{m.group(2)}", "%H:%M")),
    ]
...
def _extract_message(text: str) -> str:
    """Extract the reminder message after 'to' or 'that'."""
...
def run(query: str) -> str:
    ...
    # Combine the original query with a follow-up if the query seems incomplete
    # (no time AND no message content beyond trigger words)
    combined = q
    trigger_only = all(w in ["remind", "me", "set", "a", "reminder", "for", "an"] for w in q.split())
    has_time = _parse_absolute_time(q) is not None or _parse_delta(q) > 0

    if trigger_only or not has_time:
        speak("When and what should I remind you about?")
        follow_up = listen_once(timeout=10)
        if follow_up:
            combined = q + " " + follow_up.lower()

    # Try to parse time from combined input
    ...
    if fire_at is None:
        speak("I didn't catch a time. Try: remind me in 10 minutes to drink water.")
