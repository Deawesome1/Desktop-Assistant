"""
media_volume_windows.py — JARVIS Command (Windows)
...
COMMAND_DESCRIPTION = "Controls system volume and media playback on Windows."
...
def _set_volume(level: int) -> bool:
    try:
        subprocess.run(
            ["powershell", "-Command",
             f"(Get-AudioDevice -Playback).Volume = {level}"],
...
def _mute(state: bool) -> bool:
    try:
        cmd = "Mute" if state else "Unmute"
        subprocess.run(
            ["powershell", "-Command",
             f"(Get-AudioDevice -Playback).{cmd}()"],
            capture_output=True, timeout=4
        )
        return True
    except Exception:
